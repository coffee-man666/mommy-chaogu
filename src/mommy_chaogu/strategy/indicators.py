"""确定性指标内核：策略卡条件的当前状态评估。

用途（2026-08-12 纠偏后）：为 docs/STRATEGY-DISTILLATION-RFC.md 的策略卡提供
逐条件的当前证据（满足 / 不满足 / 无法判断），把 ma-suppression 策略卡上
「当前不可用」的云带/通道条件升级为可自动检查。不做回测、不做收益统计、
不做参数搜索——历史验证的进入条件见该 RFC §11。
