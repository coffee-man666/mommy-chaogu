...
def render_text(d: dict, session_id: str = "n/a") -> str:
    ...
    # Theme-agnostic: read theme metadata, fall back to food-security defaults
    theme = d.get("theme") or {
        "name": "粮食安全/危机",
        "name_en": "food-security",
        "basket": {"size": 35},
        "trigger_variables": [
            {"label": "FAO 谷物月度环比", "status": "✅ 触发"},
            {"label": "黑海海运量同比", "status": "✅ 触发"},
            {"label": "厄尔尼诺强度", "status": "✅ 触发"},
            {"label": "粮食主题 PE 分位", "status": "✅ 触发"},
        ],
    }
    ...
    out = []
    out.append(f"# {theme_name} · {today} {data_label} · 纯文字版")
    out.append("")
    ...
    n_total = len(triggers) or 4
    trigger_short = " / ".join((t.get("label", "—").split(" ")[0]) for t in triggers) or "4 变量"
    out.append(f"- **触发状态**: {n_met}/{n_total} {'全部' if n_met == n_total else ''}触发 ({trigger_short}), 主题{'可超配' if n_met >= 3 else '标配' if n_met == 2 else '规避'}")
    ...
    out.append("|---|---|---|---|---|")
    # v1.2: render trigger table from theme config
    out.append(f"| # | 变量 | 数据 | 阈值 | 状态 |")
    out.append(f"|---|---|---|---|")
    for i, t in enumerate(triggers, 1):
        v1 = "①②③④⑤⑥⑦⑧"[i-1] if i <= 8 else f"V{i}"
        # Try to extract "current" data
        current = t.get("current", "—")
        if t.get("label", "").startswith("FAO") and "fao_july_2026" in glob:
            current = f"{glob['fao_july_2026']} ({glob.get('fao_change_mom_pct', '?')}% MoM)"
        elif "黑海" in t.get("label", "") and "black_sea_july_yoy_pct" in glob:
            current = f"{glob['black_sea_july_yoy_pct']}% YoY"
        elif "PE" in t.get("label", "") and "fpi_pe_percentile_10y" in glob:
            current = f"{glob['fpi_pe_percentile_10y']}%"
        elif "厄尔尼诺" in t.get("label", "") and "el_nino_status" in glob:
            current = glob.get("el_nino_status", "—")
        out.append(f"| {v1} | {t.get('label', '—')} | {current} | {t.get('threshold', '—')} | {t.get('status', '—')} |")
    out.append("")
    if n_met == n_total:
        out.append(f"**含义**: {n_met}/{n_total} 全部触发, 板块整体可超配. **任一变量反转即降级**.")
    else:
        out.append(f"**含义**: {n_met}/{n_total} 触发, 主题{'可超配' if n_met >= 3 else '标配' if n_met == 2 else '规避'}.")
    out.append("")
    ...
    out.append(f"## 6. 全球{theme_short} · 4 大驱动")
    out.append("")
    out.append("| 驱动 | 当前 | 状态 |")
    out.append("|---|---|---|")
    # Global drivers — try theme's drivers, fall back to legacy food-security mapping
    drivers = theme.get("global_drivers") or [
        {"name": "FAO 食品价格指数", "stat": f"{glob.get('fao_july_2026', '?')} ({glob.get('fao_change_mom_pct', '?')}% MoM)", "desc": glob.get('fao_context', '—')},
        {"name": "厄尔尼诺", "stat": glob.get('el_nino_status', '?'), "desc": "70 年最强等级"},
        {"name": "黑海谷物出口", "stat": f"{glob.get('black_sea_july_yoy_pct', '?')}% YoY", "desc": "俄乌冲突持续压制"},
    ]
    for d in drivers[:3]:
        out.append(f"| {d.get('name', '—')} | {d.get('stat', '—')} | {d.get('desc', '—')} |")
    ...
    out.append("---")
    out.append("")
    out.append(f"**报告生成**: `basket-analysis` skill v1.2 (theme: {theme_name_en}) · session `{{session}}`")
    out.append("")
