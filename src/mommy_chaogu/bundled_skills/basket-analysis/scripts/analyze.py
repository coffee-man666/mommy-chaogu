...
Usage:
    python analyze.py --data data.json --out deliverables/{theme_name_en}/{date}/{run_ts}
...
def _get_theme(d: dict) -> dict:
    ...
    return d.get("theme") or {
        ...
        "trigger_variables": [
            ...
            {"label": "厄尔尼诺强度 (ONI)", "current": "强, 70年最强", "threshold": "≥ 1.5", "status": "✅ 触发"},
            {"label": "399365 粮食主题 PE 分位", "current": "36.5%", "threshold": "≤ 40%", "status": "✅ 触发"},
        ],
        "global_drivers": [
            {"name": "黑海出口扰动", "stat": "-40% YoY", "desc": "俄罗斯袭击敖德萨港口, 7月底黑海出货量同比 -40%。USDA 估 2026/27 美国小麦 -26%, 加拿大 -15%, 澳洲 -12%。"},
            {"name": "厄尔尼诺 70 年最强", "stat": "ONI 1.0+", "desc": "印度季风迟到, 印度洋偶极子 (IOD) 转正。澳洲冬麦预估减产 21%。WFP 警告 5000 万人可能陷入急性饥饿。"},
            {"name": "中东/霍尔木兹", "stat": "化肥 1/3", "desc": "伊朗/中东战争 → 霍尔木兹海峡封锁 → 全球约 1/3 化肥走此通道。藏格碳酸锂价格同比 +144%。粮价 → 农化大周期滞后 1-2 年启动。"},
            {"name": "北半球减产", "stat": "+25%", "desc": "小麦自 2026/1 已上涨 25%, 创 2 年新高。FAO 7 月谷物 +3.4% / 小麦 +5.8% / 玉米 +3.6% / 糖 +5.6%。"},
        ],
        "narrative_priority": "种业 > 化肥/钾磷肥 > 粮食加工贸易 > 种植 > 农药 > 饲料",
        "verification_extras": [
            "A 股农业板块是否延续强势, 国证 399365 是否突破",
            "粮食 ETF 鹏华 159698 净流入是否持续",
            "黑海/FAO 8月数据 / ONI 强度 4 变量触发是否变化",
        ],
    }
...
<meta name="skill" content="basket-analysis v1.2 (theme-agnostic)">
...
def _chrome_to_pdf(chrome: str, html_path: str, pdf_path: str, timeout: int = 30,
                  max_attempts: int = 2) -> tuple[bool, str]:
    """Render an HTML file to PDF using chrome --print-to-pdf.
    v1.2: now retries on transient failure (dbus / bluetooth / GPU noise).

    Returns (ok, error_msg). chrome-headless-shell preferred (no dbus/bluez noise).
    """
    import subprocess
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if Path(pdf_path).exists() and Path(pdf_path).stat().st_size > 500:
                if attempt > 1:
                    print(f"  ✅ PDF generated on attempt {attempt}")
                return True, ""
            last_err = r.stderr.strip()[:300] or "PDF not written"
        except subprocess.TimeoutExpired:
            last_err = f"timeout after {timeout}s"
        except Exception as e:
            last_err = str(e)[:200]
        if attempt < max_attempts:
            print(f"  ⟳ PDF attempt {attempt} failed ({last_err[:80]}...), retrying in 3s")
            import time
            time.sleep(3)
    return False, last_err or f"failed after {max_attempts} attempts"
...
def validate_data(d: dict[str, Any]) -> list[str]:
    ...
    if bf:
        n = len(bf)
        if n < basket_size:
            warnings.append(f"basket_flow 覆盖 {n}/{basket_size}, 缺 {(basket_size - n)} 只")
    t5 = d.get("top5_tech", {})
    if len(t5) < 5:
...
def render_report(d: dict[str, Any], warnings: list[str], session_id: str = "n/a") -> str:
    ...
    theme_name_en = theme.get("name_en", "theme")
    basket_size = (theme.get("basket") or {}).get("size") or 35
    ...
    n_triggers_total = max(1, len(triggers))

    md.append(f"# Mommy-chaogu · {theme_name} 主题分析报告")
    md.append(f"")
    md.append(f"**数据截至**: {today_bj} {data_label} (北京时间)")
    md.append(f"**生成时间**: {now_iso}")
    md.append(f"**Skill**: basket-analysis (theme: {theme_name_en})")
    ...
    md.append(f"| 篮子规模 | {basket_size} 只 ({(theme.get('basket') or {}).get('source', '—')}) |")
    md.append(f"| 资金流覆盖 | {len([v for v in bf.values() if v.get('main_net') is not None])}/{basket_size} |")
    md.append(f"| Top 5 MA 覆盖 | {len(t5)}/5 |")
    md.append(f"| 主题共振 ({n_triggers_total} 变量) | {n_triggers_met}/{n_triggers_total} {'✅' if n_triggers_met >= 3 else '⚠️'} |")
    md.append(f"")
    if warnings:
        md.append(f"**数据警告**:")
        for w in warnings:
    ...
    for i, t in enumerate(triggers, 1):
        circ = "①②③④⑤⑥⑦⑧"[i-1] if i <= 8 else f"{i}."
        md.append(f"| {circ} {t.get('label', '—')} | {t.get('current', '—')} | {t.get('threshold', '—')} | {t.get('status', '—')} |")
    md.append(f"")
    md.append(f"**结论: {n_triggers_met}/{n_triggers_total} 触发, 主题动能{'强' if n_triggers_met >= 3 else '中' if n_triggers_met == 2 else '弱'}**")
    md.append(f"")
    ...
    # 6. Global drivers (theme-specific)
    theme_short = theme_name.split("/")[0] if "/" in theme_name else theme_name
    md.append(f"## 6. 全球{theme_short} 4 大驱动")
    md.append(f"")
    for drv in drivers[:4]:
        md.append(f"### {drv.get('name', '—')}: {drv.get('stat', '—')}")
        md.append(f"{drv.get('desc', '—')}")
        md.append(f"")
    ...
    md.append(f"")
    # Theme-specific verification extras
    ...
    next_num = len(top5) + 1
    for ex in extras:
        md.append(f"{next_num}. {ex}")
        next_num += 1
    md.append(f"{next_num}. 重跑 `{(theme.get('basket') or {}).get('source', 'mommy')} list` 确认 {basket_size} 只都在")
    md.append(f"{next_num+1}. 限流能不能补上, 重跑 `flows pull`")
    md.append(f"{next_num+2}. memory topic 锚点是否清晰可对照")
    md.append(f"")
    ...
    md.append(f"- `deliverables/{theme.get('name_en', 'theme')}/{today_bj}.zip` (per-day 归档)")
    md.append(f"")
...
def render_strategy(d: dict[str, Any], session_id: str = "n/a") -> str:
    ...
    md.append(f"")
    md.append(f"**主题**: {theme_name} — A股配置框架")
    md.append(f"**数据截至**: {today_bj} {data_label} (北京时间)")
    md.append(f"**生成时间**: {now_iso}")
    md.append(f"**Skill**: basket-analysis (theme: {theme.get('name_en', 'theme')})")
    ...
    md.append(f"")
    triggers = theme.get("trigger_variables") or []
    trigger_labels = " + ".join(t.get("label", "—") for t in triggers) if triggers else "4 变量共振"
    priority = theme.get("narrative_priority", "高弹性子赛道")
    md.append(f"> 在 {trigger_labels}")
    md.append(f"> 这 {len(triggers)} 变量共振时, 按 **{priority}** 的优先级,")
    md.append(f"> 优先配置\"全球化定价 + 供给收缩\"或\"政策最密集 + 长期叙事\"双逻辑共振的子赛道。")
    md.append(f"")

    md.append(f"## 2. 触发条件 ({len(triggers)} 变量)")
    md.append(f"")
    md.append(f"| 变量 | 当前值 | 阈值 | 状态 |")
    md.append(f"|---|---|---|---|")
    for i, t in enumerate(triggers, 1):
    ...
    if n_met >= 3:
        md.append(f"**{n_met}/{len(triggers)} 共振 → 主题动能强, 板块整体可超配**")
    elif n_met == 2:
        md.append(f"**{n_met}/{len(triggers)} 共振 → 主题动能中, 板块标配**")
    else:
        md.append(f"**{n_met}/{len(triggers)} 共振 → 主题动能弱, 板块规避**")
    md.append(f"")
    ...
    md.append(f"| **FACT** | 主题指数前十大权重 + PE 分位 |")
    md.append(f"| **FACT** | 4 变量当前值 (来自权威数据源, 见报告 §1) |")
...
def main() -> int:
    ...
    # v1.2 retry: if basket_flow is missing some codes (efinance rate limit),
    # retry the upstream pull with backoff. We only retry for codes the theme
    # actually wants (filtered set), to avoid wasting calls on unrelated groups.
    # Opt-in via env var: BASKET_RETRY=1
    import os
    if os.environ.get("BASKET_RETRY", "0") == "1":
        # Get the desired codes (theme's basket)
        # We re-compute since `theme_codes` is defined later
        _theme = _get_theme(d)
        _basket = _theme.get("basket") or {}
        _codes = list(_basket.get("codes") or [])
        if not _codes and _basket.get("group_name"):
            try:
                import sqlite3
                _conn = sqlite3.connect(str(Path("/workspace/.home/.local/share/mommy-chaogu/portfolio.db")))
                for _r in _conn.execute("SELECT code FROM stock_entries e JOIN groups g ON e.group_id=g.id WHERE g.name=?", (_basket["group_name"],)):
                    _codes.append(_r[0])
                _conn.close()
            except Exception:
                pass
        if _codes:
            ...
            print(f"  ⏳ retry-mode: re-pulling money flow for {len(_codes)} codes (with backoff)...")
            retry_money_flow(_codes, max_attempts=2, initial_delay=5.0)

    # v1.2 fix: filter basket_flow to ONLY the theme's basket codes (was including
    # any cached code from other groups, polluting the report)
    # Resolution order:
    #   1) d["theme"]["basket"]["codes"] (explicit list, recommended)
    #   2) d["theme"]["basket"]["group_name"] (look up codes by group name in mommy DB)
    #   3) d["theme"]["basket"]["source"] (parse CLI form, best-effort)
    theme_codes = []
    basket_meta = _get_theme(d).get("basket") or {}
    if basket_meta.get("codes"):
        theme_codes = list(basket_meta["codes"])
    elif basket_meta.get("group_name"):
        try:
            import sqlite3
            conn = sqlite3.connect(str(Path("/workspace/.home/.local/share/mommy-chaogu/portfolio.db")))
            for r in conn.execute("SELECT code FROM stock_entries e JOIN groups g ON e.group_id=g.id WHERE g.name=?", (basket_meta["group_name"],)):
                theme_codes.append(r[0])
            conn.close()
        except Exception:
            pass
    elif basket_meta.get("source"):
        # best-effort: try to match by CLI/group name in DB
        src = basket_meta["source"]
        # Extract likely group name from "mommy <cli>" or "mommy watchlist <name>"
        group_name = None
        if src.startswith("mommy "):
            group_name = src.split("mommy ", 1)[-1]
        elif src.startswith("mommy watchlist "):
            group_name = src.split("mommy watchlist ", 1)[-1]
        # Map common CLI names to actual group names
        cli_to_group = {
            "food-security": "粮食安全-全量",
            "粮食安全": "粮食安全-全量",
            "semicon": None,  # uses semicon_db, not stock_entries
        }
        actual_name = cli_to_group.get(group_name, group_name)
        if actual_name:
            try:
                import sqlite3
                conn = sqlite3.connect(str(Path("/workspace/.home/.local/share/mommy-chaogu/portfolio.db")))
                for r in conn.execute("SELECT code FROM stock_entries e JOIN groups g ON e.group_id=g.id WHERE g.name=?", (actual_name,)):
                    theme_codes.append(r[0])
                conn.close()
            except Exception:
                pass
    if theme_codes and d.get("basket_flow"):
        theme_codes_set = set(theme_codes)
        before = len(d["basket_flow"])
        d["basket_flow"] = {k: v for k, v in d["basket_flow"].items() if k in theme_codes_set}
        theme_main = _get_theme(d)
        d["basket_total"] = (theme_main.get("basket") or {}).get("size") or 35
        after = len(d["basket_flow"])
        if before != after:
            print(f"  ℹ️  filtered basket_flow {before} → {after} (only theme basket {d['basket_total']} codes)")
