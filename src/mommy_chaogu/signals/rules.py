    ...
    def evaluate(self, snapshot: Snapshot) -> list[Signal]:
        ...
        try:
            ...
            logging.getLogger(__name__).error(
                "rule %s evaluate failed: %s", self.rule_id, e,
            )
            return []
    ...
    def _evaluate(self, snapshot: Snapshot, config: RuleConfig) -> list[Signal]:
        ...
        for row in snapshot.rows:
            ...
            out.append(Signal(
                ...
                detail=(
                    f"开盘 {q.open}, 昨收 {q.prev_close}, "
                    f"缺口 {q.open - q.prev_close:+.2f}"
                ),
...
class PortfolioBreadthRule(RuleBase):
    ...
    def _evaluate(self, snapshot: Snapshot, config: RuleConfig) -> list[Signal]:
        ...
        out.append(Signal(
            ...
            metrics={
                "n_codes": n, "n_up": n_up, "n_down": n_down,
                "pct_up": pct_up, "pct_down": pct_down,
            },
        ))
        return out
...
class PortfolioMainFlowRule(RuleBase):
    ...
    def _evaluate(self, snapshot: Snapshot, config: RuleConfig) -> list[Signal]:
        ...
        return [Signal(
            ...
            detail=(
                f"涨 {snapshot.n_up} 跌 {snapshot.n_down}, "
                f"总成交 {snapshot.n_codes} 只"
            ),
