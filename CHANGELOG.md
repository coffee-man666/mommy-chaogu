...
- **Web 引导式配置**——首次打开 `/setup` 逐步完成 Provider 选择、Key 验证和可选微信扫码，替代裸命令行 `mommy setup`。
- **产品 UX 整体执行**——统一 baskets 与 today 决策摘要；新增个股决策上下文（行情 + 资金流 + 业绩 + 预测聚合）；服务端共享用户偏好（主题、风险偏好等）跨设备同步。
- **预测证据 + 个股回测**——预测卡片附带证据链（触发时的行情快照）；新增回测流程可对单只股票跑历史预测评分。
- **CLI 助手体验升级**——增强默认 CLI 助手的交互体验和提示。

### 变更

- 移除 Nova Bridge provider（已废弃）。

### 修复

- **CLI 首字符吞输**——首次提示输入时光标位置导致第一个字符被删除。
- **预测附件与偏好迁移**——强化预测证据的持久化写入，修正偏好 schema 迁移边界问题。
- **主题批量报价**——主题页批量拉取报价时缓存条目数无上限，改为按需裁剪。
- **安装脚本输出**——收敛 install.sh 日志噪音。
- **安全同步**——`cryptography` 升至 50.0.0（修复 PYSEC-2026-3552）；前端打包产物与源码构建重新同步。
