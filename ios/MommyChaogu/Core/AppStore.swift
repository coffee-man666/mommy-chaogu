...
final class AppStore: ObservableObject {
    ...
    init() {
        ...
        self.theme = theme; self.configuration = config; self.conversations = conversations
    ...
    func loadPortfolio() async {
        ...
        catch { if case .idle = connection { connection = .failed(error.localizedDescription) } }
    }
    ...
    func send(_ raw: String) async {
        ...
        updateConversation(with: text)
        do {
            try await client.streamAgent(message: text, sessionID: activeConversationID) { event in
        ...
        } catch { messages[responseIndex].content = error.localizedDescription }
        isResponding = false; activeTools = []
        if messages[responseIndex].content.isEmpty { messages[responseIndex].content = "这次没有收到回复，请稍后再试。" }
        updateConversation(with: messages[responseIndex].content)
    }

    private func updateConversation(with text: String) {
