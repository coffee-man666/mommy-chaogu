...
def _seed_bars(store: CacheStore, code: str, closes: list[str | float], name: str = "测试股") -> None:
    """写入 DATES 对应的日 K 线。"""
    for date, close in zip(DATES, closes, strict=False):
        store.set_bar(code, "1d", "forward", date, _make_bar(code, date, close, name))


def _seed_flows(store: CacheStore, code: str, date: str, main_net: int, name: str = "测试股") -> None:
    """写入一天的资金流数据。"""
...
def test_result_fields_present(engine: BacktestEngine, store: CacheStore):
    ...
    sig = result.signals_detail[0]
    for key in ("code", "name", "date", "ratio_bp", "return_after_1d",
                "return_after_3d", "return_after_5d", "return_after_hold_pct"):
