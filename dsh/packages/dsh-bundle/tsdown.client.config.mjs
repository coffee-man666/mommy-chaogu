/**
 * External client-bundle tsdown config（dsh-trading client-ui-trading 同款
 * 三段 banner/intro/footer 契约 + 构建期纯度门禁）。
 *
 * 平台种子词（宿主 packages/client/web/src/seed.ts，0.1.5-rc.2 共 9 个）：
 * 浏览器 require 只解析这 9 个 + 各插件自身 factory。本包运行时 import 的
 * SDK 值为零（类型走本地结构面），CSS Modules 经 lightningcss 内联。
 */
import { readFile } from 'node:fs/promises'
import { basename, dirname, resolve } from 'node:path'
import { transform } from 'lightningcss'

const ID = '@mommy-chaogu/dsh-bundle'

// 宿主 seed.ts 的平台词表（单一真相源在宿主；此处同步副本，升级宿主时核对）。
...
const EXTERNALS = new Set(PLATFORM_EXTERNALS)
const isExternal = (specifier) => EXTERNALS.has(specifier)

const purityGate = () => ({
  name: 'mommy-chaogu-client-purity',
  resolveId(source) {
    if (!source.startsWith('@deepseek-ai/')) return null
    if (isExternal(source)) return null
    if (/^@deepseek-ai\/(?:cosmokit|schemastery)(?:\/|$)/.test(source)) return null
    throw new Error(
      `client bundle purity: "${source}" is not in the externals list — declare it in dsh.client.external or collaborate through cordis services`,
    )
  },
})

const cssModulesInline = () => ({
  name: 'mommy-chaogu-css-modules-inline',
...
const cssGlobalInline = () => ({
  name: 'mommy-chaogu-css-global-inline',
  resolveId(source, importer) {
  ...
  async load(virtualId) {
    ...
    const { code } = transform({
      filename: fileId,
      code: Buffer.from(await readFile(fileId, 'utf8'), 'utf8'),
      minify: true,
    })
    const tagId = `${ID}/${basename(fileId)}`
    const css = JSON.stringify(code.toString())
