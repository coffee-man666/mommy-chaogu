...
**可由用户现有 Agent 接管的本地行情监控 + 投资陪伴应用。**

- **核心场景**：用户看行情、看资金流、看板块联动；把文章或自己的方法沉淀为可复用的
  策略卡；系统在关键时刻主动提醒。
- **核心约束**：
...
本项目是最终用户直接使用的应用，不是供其他应用复用的工程框架。先定义用户能完成什么、能看到
什么以及为什么值得信任，再建设支撑它的最小工程。schema、runtime、测试矩阵和可复现 hash 只能
证明支撑代码的某些性质，不能替代用户完成事件。Draft RFC、示例和 fixture 只用于验证已确认的
产品目标，不能反过来定义产品。
