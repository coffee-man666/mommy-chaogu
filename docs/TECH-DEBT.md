...
> 发布 v0.2.0 前的如实记录：质量门的真实覆盖范围与已知遗留项。
...
| `mommy_chaogu.agent.{service, tools, mcp_server, extractor, consolidator, monitor, narrative, vector_search}` | OpenAI / MCP 动态响应对象 | 在边界定义窄类型（TypedDict）后向内推进 |
...
- **258 个 DeprecationWarning**（v0.2.0 实测）：`sqlite3` 默认 datetime /
