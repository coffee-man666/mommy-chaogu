...
最后更新：2026-07-04（数据源调研 + 多模型对比完成，`memory-system-v1` 分支）
...
| **基准命中率** | **53%（154 条，规则引擎）** | **50%（68 条方向性，glm-5 最佳）** |
...
### 3.6 多模型横向对比 trial_2（5 个 GLM 模型，已完成）

> 用 z.ai API 的 5 个 GLM 模型对同一批 70 条数据做回测，横向对比命中率、分析风格、
> token 消耗。每条预测完整记录了 prompt / LLM 回复 / 方向 / 理由 / token / 命中状态。

**数据**：10 只半导体链股票 × 7 个日期 = 70 条预测（T+5 验证）

#### 横向对比总表

| 模型 | 方向性 | Neutral | **总命中率** | **Bullish%** | **Bearish%** | Token | 成本¥ |
|---|---|---|---|---|---|---|---|
| glm-4.7 | 61 | 9 | **45.9%** | **95.8%** | 13.5% | 179,662 | 0.36 |
| **glm-5** | 68 | 2 | **50.0%** | **93.1%** | **17.9%** | 104,076 | 0.21 |
| glm-5-turbo | 66 | 4 | 43.9% | 91.7% | 16.7% | 102,653 | 0.21 |
| glm-5.1 | 68 | 2 | 44.1% | 92.3% | 14.3% | 106,732 | 0.21 |
| glm-5.2 | 66 | 4 | 47.0% | 89.3% | 15.8% | 101,191 | 0.20 |

> **glm-5 总命中率最高（50.0%）**，bearish 命中率也最高（17.9%）。
> **glm-4.7 bullish 最准（95.8%）**，但 token 消耗是其他模型的 1.7 倍。
> **glm-5.2 最省 token（101K）**，命中率与 glm-5 接近（47.0%）。

#### 跨模型一致性发现

所有 5 个模型展现**高度一致的系统性偏差**：

1. **Bullish 准确率极高（89-96%）**：当 LLM 判断看涨时，5 天后确实大概率上涨
2. **Bearish 命中率极低（13-18%）**：看跌判断几乎全错——因为回测期间（2026 年 6 月）
   半导体板块处于强势上涨期，任何短期看跌判断都被趋势反复打脸
3. **预测倾向偏空**：所有模型给出 53-60% bearish 预测，但实际行情偏多
4. **neutral 是成熟模型的优势**：glm-4.7 给出 9 条 neutral（最谨慎），
   glm-5/5.1 只有 2 条（最激进）

#### 分个股命中率（跨模型对比）

| 股票 | glm-4.7 | glm-5 | glm-5-turbo | glm-5.1 | glm-5.2 |
|---|---|---|---|---|---|
| 002371 北方华创 | **6/7 (86%)** | **6/7 (86%)** | **6/7 (86%)** | **6/7 (86%)** | **6/7 (86%)** |
| 002156 通富微电 | 3/4 (75%) | 4/7 (57%) | 3/6 (50%) | 4/7 (57%) | 4/7 (57%) |
| 002549 凯美特气 | 4/6 (67%) | 4/6 (67%) | 3/7 (43%) | 4/7 (57%) | 4/7 (57%) |
| 002129 TCL中环 | 2/6 (33%) | 4/7 (57%) | 3/6 (50%) | 2/5 (40%) | 3/6 (50%) |
| 002409 雅克科技 | 2/6 (33%) | 4/7 (57%) | 3/7 (43%) | 3/7 (43%) | 4/7 (57%) |
| 300054 鼎龙股份 | 3/6 (50%) | 3/7 (43%) | 2/7 (29%) | 2/7 (29%) | 3/7 (43%) |
| 300398 飞凯材料 | 3/6 (50%) | 3/7 (43%) | 2/6 (33%) | 3/7 (43%) | 1/5 (20%) |
| 002049 紫光国微 | 2/7 (29%) | 2/6 (33%) | 3/7 (43%) | 2/7 (29%) | 2/7 (29%) |
| 300346 南大光电 | 2/7 (29%) | 2/7 (29%) | 2/7 (29%) | 2/7 (29%) | 2/6 (33%) |
| 000021 深科技 | 1/6 (17%) | 2/7 (29%) | 2/6 (33%) | 2/7 (29%) | 2/7 (29%) |

> **北方华创（002371）是唯一所有模型都 86% 命中的股票**——趋势性极强，
> LLM 和规则引擎都容易判断正确。**深科技（000021）是所有模型表现最差的**。

#### LLM 分析风格差异

以「北方华创 @ 2026-06-18」为例（所有模型都 bullish + hit 1.0）：

| 模型 | 理由摘要 | Token |
|---|---|---|
| glm-4.7 | "放量突破后的强势上升通道，连续突破 700 及 720 整数关口；主力净流入超 7 亿" | 1379 |
| glm-5 | "沿均线稳步攀升的多头排列，量价配合良好；净流入超 7.4 亿元（占比 9.9%）" | 1288 |
| glm-5-turbo | "明显的多头上升通道，近两日放量长阳突破，主力资金连续大幅净流入" | 551 |
| glm-5.1 | "K线走出强势上升趋势，连续创出阶段新高；6/17 和 18 连续放量上攻" | 1015 |
| glm-5.2 | "多头排列，量价齐升，股价创出阶段新高；基准日净流入超 7 亿元" | 1063 |

> glm-5-turbo 最精炼（551 token），glm-4.7 最详细（1379 token）。
> 所有模型都抓住了「量价齐升 + 主力流入」的核心逻辑。

#### 过程记录

每条预测的完整记录（prompt / LLM 原始回复 / 解析方向 / 理由 / token / 实际涨跌 / 命中状态）
存储在 `/tmp/backtest_multi/results_{model}.json`，5 个文件共 ~2,500 行。

**回测脚本**：`/tmp/backtest_multi/run_model.py`（参数化 model name，可复现）

```bash
# 复现单个模型回测
uv run python /tmp/backtest_multi/run_model.py glm-5

# 串行跑全部 5 个模型
for m in glm-4.7 glm-5 glm-5-turbo glm-5.1 glm-5.2; do
    uv run python /tmp/backtest_multi/run_model.py $m
done
```

#### vs 规则引擎 + Agent 原生

| 方法 | 总命中率 | Bullish% | Bearish% | 成本 | Neutral |
|---|---|---|---|---|---|
| 规则引擎（154 条） | **53%** | 41% | **57%** | ¥0 | ❌ |
| Agent 原生（19 条） | 47% | **88%** | 18% | ¥0 | ✅ 6 条 |
| glm-5 最佳（68 条） | **50%** | **93%** | 18% | ¥0.21 | ✅ 2 条 |
| glm-4.7 最精（61 条） | 46% | **96%** | 14% | ¥0.36 | ✅ 9 条 |
...
2. **样本小** —— 规则引擎 154 条 / LLM 回测 350 条（5 模型 × 70），统计显著性有限。
   扩展到更多股票和更长时间窗口后可做稳健性检验。
...
5. **Benchmark 对照** —— 后续加入「等权持有」「均线择时」等 naive 基线，量化规则
   引擎和 LLM 各自的 alpha。
6. **Bearish 系统性偏差** —— 所有 5 个 GLM 模型的 bearish 命中率仅 13-18%，因为回测
   期间（2026 年 6 月）半导体板块整体强势上涨。需要在下跌区间验证 bearish 策略。
7. **数据源限制（核心瓶颈）** —— 见下方 §6 详细说明。

---

## 6. 数据源现状与后续扩展（2026-07-04 调研）

### 当前可用数据源（从美国 IP）

| 数据 | 源 | 状态 | 历史深度 | 说明 |
|---|---|---|---|---|
| **日 K 线（OHLCV）** | 腾讯 `web.ifzq.gtimg.cn` | ✅ 可用 | **2024-01 至今（604 天）/ 2025-01 至今（362 天）** | 支持所有 A 股 + 指数 |
| 实时报价 | 腾讯 `qt.gtimg.cn` | ✅ 可用 | 实时 | 项目 TencentAdapter 已集成 |
| 实时报价 | 东财 `push2delay.eastmoney.com` | ✅ 可用 | 实时 | 不封美国 IP |
| 东财数据中心 | `datacenter-web.eastmoney.com` | ✅ 可用 | — | 财务数据可用，无资金流报表 |

### 不可用数据源

| 数据 | 源 | 状态 | 原因 |
|---|---|---|---|
| **历史资金流** | 东财 `push2/push2his.eastmoney.com` | ❌ 封锁 | 美国IP被拒（所有 push2 子域名） |
| 历史资金流 | 新浪 `vip.stock.finance.sina.com.cn` | ❌ 接口失效 | 返回 Input error |
| 历史资金流 | 雪球 `stock.xueqiu.com` | ❌ 需要登录 | 400 错误 |
| emdata-proxy | Railway 新加坡代理 | ❌ 500 错误 | 代理服务端返回 Internal Server Error |

### 扩展方向（优先级排序）

1. **修复 emdata-proxy** —— 代理部署在 Railway（新加坡），能绕过东财 IP 封锁。
   当前返回 500，需检查 Railway 日志排查上游连接问题。修复后可拿东财全部历史资金流。
2. **国内服务器定时采集** —— 在国内 Mac mini 或云服务器上跑定时脚本，拉东财资金流
   推送到美国端，数据缓存到 `data/market.db`。
3. **纯 K 线扩展回测** —— 不等资金流，用腾讯 K 线从 2025 年开始覆盖沪深 300 / 科创板 /
   创业板成分股，用量价指标（OBV、量比、换手率代理）替代资金流信号。
4. **长桥 OpenAPI** —— 支持 A 股 K 线（1999 至今）+ 当日资金流，但需要开户 + 每月
   配额限制（免费 100 只/月）。历史资金流不提供。
