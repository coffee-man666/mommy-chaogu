...
当 Kimi Code / Claude 在项目目录里操作时，它的能力边界是整个文件系统。这对**开发这个项目**是合理的（要能改代码、跑测试、写文件），但直接把这种"自由读写文件系统"的模式当**投研入口**是危险的（可能误删数据库、改错配置）。
...
## 进化记录：v3 — MemoryService 抽象 + MCP 记忆打通

### 问题：记忆管道耦合在 AgentService 内部

`AgentService.chat()` 同时做了三件事：记忆注入（`build_prompt`）、LLM 推理（工具循环）、记忆提取（`record_analysis`）。后两个是 AgentService 的核心职责，但第一个和第三个不应该绑死在这里——因为 MCP Server 不调 `chat()`，所以拿不到记忆。

这是设计层面的耦合，不是 bug。解决方式是提取 MemoryService。
...
改造前：
AgentService.chat()
  ├─ pipeline.build_prompt()      ← 内部
  ├─ LLM ↔ 工具循环
  ├─ memory.add()                 ← 内部
  └─ pipeline.record_analysis()   ← 内部

改造后：
MemoryService
  ├─ get_context(query) → str
  ├─ record_conversation(user, assistant)
  └─ stats() → dict

AgentService.chat()
  ├─ memory_service.get_context()     ← 调服务
  ├─ LLM ↔ 工具循环
  └─ memory_service.record_conversation() ← 调服务

MCP Server (ToolContext.memory_service)
  └─ get_memory_context 工具           ← 外部 LLM 主动调
```

### MCP 记忆打通

外部 LLM（如 Claude Desktop）现在可以通过 `get_memory_context` 工具获取项目积累的全部历史记忆——317 条事件、136 条预测（49% 命中率）、10 条语义知识。这让 MCP 模式和内置 Agent 模式获得了**一致的记忆数据**。

**不改变的**：分析质量取决于 LLM 本身——DeepSeek 和 Claude 的推理风格不同，但两边看到的茅台历史预测、资金流命中率是一样的。

### 设计教训

> 记忆是基础设施，不是某个组件的内部逻辑。任何需要"记住过去"的入口都应该能访问同一份记忆。

如果把记忆管道留在 AgentService 里，每加一个新入口就要重新实现一遍记忆逻辑。提取为 MemoryService 后，新入口只需要构造一个 MemoryService 实例。
